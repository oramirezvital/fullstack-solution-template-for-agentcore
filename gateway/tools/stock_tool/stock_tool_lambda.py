...
def get_stock_information(symbol: str, api_key: str) -> Dict[str, Any]:
    ...
    try:
        ...
        stock_data = {
            ...
            'daily_change_percent': float(quote.get('10. change percent', '0').replace('%', '')),
            'volume': int(quote.get('06. volume', 0)),
            'market_cap': int(overview_data.get('MarketCapitalization', 0)) if overview_data.get('MarketCapitalization', '0').isdigit() else 'N/A',
            'day_high': float(quote.get('03. high', 0)),
            'day_low': float(quote.get('04. low', 0)),
            'week_52_high': float(overview_data.get('52WeekHigh', 0)) if overview_data.get('52WeekHigh') else 'N/A',
            'week_52_low': float(overview_data.get('52WeekLow', 0)) if overview_data.get('52WeekLow') else 'N/A',
            'pe_ratio': float(overview_data.get('PERatio', 0)) if overview_data.get('PERatio') and overview_data.get('PERatio') != 'None' else 'N/A'
        }
        
        return stock_data
        
    except urllib.error.HTTPError as e:
        logger.error(f"HTTP error fetching stock data for {symbol}: {e.code} - {e.reason}")
    ...
    try:
        ...
        if tool_name == "get_stock_info":
            ...
            if not symbol:
                ...
                return {"error": "Missing required parameter 'symbol'. Please provide a stock ticker symbol (e.g., 'AAPL', 'GOOGL')."}
